...
function App() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setMessage({ type: 'error', text: 'Failed to submit URL. Try again.' });
  ...
  const fetchMentors = async () => {
    setTimeout(() => {
      const mentorData = [
        { name: 'Mentor 1', expertise: 'Software Engineering', image: 'https://randomuser.me/api/portraits/men/1.jpg', rating: 4 },
        { name: 'Mentor 2', expertise: 'Data Science', image: 'https://randomuser.me/api/portraits/women/1.jpg', rating: 5 },
        { name: 'Mentor 3', expertise: 'AI/ML', image: 'https://randomuser.me/api/portraits/men/2.jpg', rating: 3 },
        { name: 'Mentor 4', expertise: 'Web Development', image: 'https://randomuser.me/api/portraits/men/3.jpg', rating: 4 },
        { name: 'Mentor 5', expertise: 'Cybersecurity', image: 'https://randomuser.me/api/portraits/women/2.jpg', rating: 5 },
      ];
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="flex-1 p-6 md:p-12 text-center">
        ...
        {/* Message Feedback */}
        {message && (
          <div className={`mt-2 p-2 rounded-md text-white ${message.type === 'success' ? 'bg-green-500' : 'bg-red-500'}`}>
            {message.text}
          </div>
        )}
          ...
          mentors.length > 0 && (
            <div className="grid grid-cols-1 sm:grid-cols-2 md-grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 w-full max-w-6xl mx-auto">
              {mentors.map((mentor, index) => (
                <MentorCard key={index} name={mentor.name} expertise={mentor.expertise} image={mentor.image} rating={mentor.rating} />
              ))}
            </div>
          )
        )}
